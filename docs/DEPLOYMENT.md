...
### V1.0.1 Production acceptance record

The owner completed this gate on 24–25 September 2026. The pre-migration application ZIP and custom-format PostgreSQL dump were stored off the POS and integrity-checked without exposing credentials. Migrations 10 and 11 were applied with `prisma migrate deploy`, Production reported 11/11 migrations, and the reviewed `main` release was deployed to Vercel Mumbai (`bom1`). Authenticated Production smoke tests, Staff Management, Settings/Danger Zone, Windows touchscreen, and XP-80T browser printing passed.

The owner then executed Start Fresh manually after confirming the backup and closed register. Read-only verification found the cutoff set, one matching backup-confirmed `START_FRESH` audit, retained pre-cutoff invoices/quotation/cash sessions, a new current-period invoice, an audited ADMIN password reset, and no open cash session. No Start Fresh operation was run by deployment automation.
