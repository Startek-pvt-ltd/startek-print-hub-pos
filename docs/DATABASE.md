...
Migration `202609190001_add_operational_data_start` is the tenth immutable migration and adds nullable `settings.operationalDataStartAt TIMESTAMP(3)`. Null means all retained history is operationally visible; a UTC timestamp archives older operational records from normal views without deleting them. It adds no cascade, trigger, reset table, product model, or counter change.

Migration `202609210001_add_username_and_staff_role` is the eleventh immutable migration. It additively appends `STAFF` to the existing `Role` enum, retains all legacy enum values, adds a required normalized unique `users.username`, assigns `stadmin` to the existing primary active ADMIN row, and gives other retained legacy identities stable collision-safe compatibility usernames. It does not delete or recreate users, so audit, invoice, expense, cash, quotation, and order relations remain unchanged. A database check enforces lowercase 3–32 character usernames using letters, digits, dots, underscores, or hyphens.

Both migrations were applied to the separate Production Supabase project with `prisma migrate deploy` on 24 September 2026. Production reported 11/11 migrations applied with valid history and schema up to date; no applied migration was rewritten and `prisma db push` was not used.
