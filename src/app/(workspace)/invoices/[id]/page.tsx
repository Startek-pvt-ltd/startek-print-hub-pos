...
export default async function InvoiceDetailPage({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ error?: string }> }) {
  ...
  const [invoice, cutoff] = await Promise.all([getInvoiceById(id), getOperationalDataStartAt()]);
  if (!invoice) notFound();
  const archived = isArchivedOperationalRecord(invoice.createdAt, cutoff);
