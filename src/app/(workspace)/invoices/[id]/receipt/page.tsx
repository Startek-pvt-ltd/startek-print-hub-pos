...
export default async function ReceiptPage({
  ...
  searchParams: Promise<{ reprint?: string; autoprint?: string }>;
}) {
  const user = await requirePermission("invoices:view");
  const [{ id }, query] = await Promise.all([params, searchParams]);
  const [invoice, settings, cutoff] = await Promise.all([
    getInvoiceById(id),
    db.setting.findUnique({ where: { id: "primary" } }),
    getOperationalDataStartAt(),
  ]);
  if (!invoice || !settings) notFound();
  if (isArchivedOperationalRecord(invoice.createdAt, cutoff) && user.role !== "ADMIN") notFound();
  const reprintAudit = query.reprint
    ...
    : null;
  const receipt = buildReceiptViewModel(
    invoice,
    settings,
    Boolean(reprintAudit),
  );

  return (
    <div className="receipt-print-page mx-auto max-w-[80mm]">
      ...
      <div className="mb-5 print:hidden">
        <PrintButton autoPrint={query.autoprint === "1"} />
      </div>
