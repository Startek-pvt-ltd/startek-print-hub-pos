...
export async function GET(_request: Request, { params }: { params: Promise<{ id: string }> }) {
  ...
  try {
    const [invoice, settings, logo] = await Promise.all([
    ...
    if (!settings) return new Response("Business settings unavailable", { status: 503 });
    const bytes = await generateInvoicePdf(buildReceiptViewModel(invoice, settings), logo);
    ...
    return new Response("Unable to download invoice PDF. Please try again.", { status: 500 });
  }
}
