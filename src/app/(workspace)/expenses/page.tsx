...
export default async function ExpensesPage({ searchParams }: { searchParams: Promise<Record<string, string | string[] | undefined>> }) {
  ...
  const [raw, cutoff] = await Promise.all([searchParams, getOperationalDataStartAt()]); const value = (key: string) => typeof raw[key] === "string" ? raw[key] : ""; const search = value("search").trim();
  ...
  const effectiveFrom = cutoff ? effectiveOperationalStart(from ?? new Date(0), cutoff) : from;
  const where: Prisma.ExpenseWhereInput = { category, paymentMethod: method, expenseDate: effectiveFrom || to ? { gte: effectiveFrom, lte: to } : undefined, OR: search ? [{ expenseNumber: { contains: search, mode: "insensitive" } }, { description: { contains: search, mode: "insensitive" } }] : undefined };
