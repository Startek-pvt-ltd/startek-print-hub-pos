...
export default async function ExpenseDetailPage({
  ...
  const [expense, cutoff] = await Promise.all([
  ...
  if (!expense) notFound();
  const archived = isArchivedOperationalRecord(expense.createdAt, cutoff);
  ...
  return (
    <div className="mx-auto max-w-4xl">
      ...
      <PageHeading
        ...
        description={`${expenseCategoryLabels[expense.category]} · ${paymentMethodLabels[expense.paymentMethod]}`}
      ...
      <div className="grid gap-5 md:grid-cols-2">
        <Card className="p-6">
          <dl className="grid gap-5">
            ...
            <Detail label="Amount" value={`Rs. ${expense.amount.toFixed(2)}`} />
            <Detail
              label="Date and time"
              value={formatShopDateTime(expense.expenseDate)}
            />
            <Detail label="Entered by" value={expense.createdBy.name} />
            {expense.cashSession ? (
              <Detail
                label="Cash session"
                value={
                  <Link
                    ...
                    href={`/cash-register/${expense.cashSession.id}`}
                  >
                    View session · {expense.cashSession.status}
                  </Link>
                }
              />
            ) : null}
        ...
        <Card className="p-6">
          {expense.status === "VOID" ? (
            <div className="space-y-4">
              <h2 className="font-black text-rose-800">Void context</h2>
              <Detail label="Reason" value={expense.voidReason ?? "—"} />
              <Detail label="Voided by" value={expense.voidedBy?.name ?? "—"} />
              <Detail
                label="Voided at"
                value={
                  expense.voidedAt ? formatShopDateTime(expense.voidedAt) : "—"
                }
              />
            </div>
          ) : archived ? (
            <p className="rounded-xl bg-amber-50 p-4 text-sm font-semibold text-amber-900">
              Archived expenses are read-only.
            </p>
          ) : expense.cashSession?.status === "CLOSED" ? (
            <p className="rounded-xl bg-slate-100 p-4 text-sm">
              This cash expense belongs to a closed register and is immutable.
            </p>
          ) : (
