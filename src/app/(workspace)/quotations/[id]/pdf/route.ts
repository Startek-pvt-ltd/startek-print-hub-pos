...
import { generateQuotationPdf } from "@/server/quotation-pdf";
...
export async function GET(
  ...
  const [quotation, settings, logo, cutoff] = await Promise.all([
  ...
  if (isArchivedOperationalRecord(quotation.createdAt, cutoff) && user.role !== "ADMIN") return new Response("Quotation not found", { status: 404 });
  const bytes = await generateQuotationPdf(
    {
      quotationNumber: quotation.quotationNumber,
      createdAt: quotation.createdAt,
      customerName: quotation.customerNameSnapshot,
      customerPhone: quotation.customerPhoneSnapshot,
      ...
      business: {
        name: settings?.businessName ?? "Startek Print Hub",
        address: settings?.address ?? "No.62 Padukka Road, Meegoda",
        phones: [settings?.phonePrimary, settings?.phoneSecond]
          .filter(Boolean)
          .join(" / "),
        email: settings?.email ?? "startekprinthub@gmail.com",
      },
