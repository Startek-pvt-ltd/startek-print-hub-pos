import { calculateInvoiceTotals, validatePayment } from "@/domain/financial";
...
export function prepareInvoicePlan(input: InvoiceInput) {
  ...
  const initialPayment = input.initialPayment ? validatePayment(input.initialPayment.amount, totals.grandTotal) : null;
